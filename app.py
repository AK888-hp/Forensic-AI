...
```
    ...
    for msg in st.session_state.chat_history:
        with st.chat_message(msg["role"]):
            ...
            if msg.get("warnings"):
                st.markdown(f'<div class="warning">⚠️ Flags: {", ".join(msg["warnings"])}</div>',
                           unsafe_allow_html=True)
            if msg.get("blocked"):
                st.markdown(f'<div class="blocked">🚫 Blocked</div>',
                           unsafe_allow_html=True)
        ...
        with st.spinner("Investigating..."):
            try:
                ...
                try:
                    ...
                    # n8n not running — go directly to Python API
                    resp = requests.post(
                    ...
                    raw = resp.json()
                    result = {
                        "final_response": raw.get("answer", "No answer returned"),
                        "blocked": False,
                        "warnings": ["n8n not connected — using direct API"],
                        "stages": {}
                    }
                ...
                st.session_state.chat_history.append({
                    "role": "assistant",
                    "content": result.get("final_response", "No response"),
    ...
    if st.button("🔄 Refresh Logs"):
        try:
            ...
            if logs:
                ...
                with col2:
                    blocked = df[df.get("blocked", False) == True] if "blocked" in df else pd.DataFrame()
                    st.metric("Blocked", len(blocked))
                with col3:
                    st.metric("Allowed", len(df) - len(blocked))
                with col4:
...
# Perfect for showing the professor
# ══════════════════════════════════════
with tab5:
    st.subheader("🛡️ Live Guardrail Demonstration")
    st.info("Try these queries to demonstrate guardrails working in real time")
    ...
    for label, demo_query in demo_queries.items():
        ...
        with col2:
            if st.button(f"Test →", key=label):
                ...
                with st.spinner("Running through guardrails..."):
                    try:
